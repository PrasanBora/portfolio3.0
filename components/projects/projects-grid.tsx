  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {isLoading
          ? Array.from({ length: 6 }).map((_, i) => <Card key={i} className="h-48 animate-pulse" aria-hidden />)
          : filtered.map((p, idx) => (
              <motion.div
                key={p.id}
                ...
                <Card className="h-full flex flex-col overflow-hidden">
                  ...
                  <CardContent className="flex-1">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {p.tech.map((t) => (
                        <span key={t} className="rounded bg-muted text-muted-foreground px-2 py-0.5 text-xs">
                          {t}
                        </span>
                      ))}
                    </div>
