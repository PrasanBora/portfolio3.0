    ...
    <motion.section
      ...
      <div
        ...
        <Image
          src={
            imageSrc ||
            "/placeholder.svg?height=800&width=1600&query=work%20hero%20image"
          }
          alt={imageAlt ?? ""}
          fill
          priority={false}
      ...
      <div className="relative z-10 grid grid-cols-1 items-center gap-6 p-6 md:grid-cols-2 md:p-12 transition-opacity duration-300 ease-out group-hover:opacity-0 group-focus-within:opacity-0">
        <div className="max-w-xl text-foreground">
          {displayCategory ?
            <p className="mb-2 text-sm/5 tracking-wide opacity-90">
              {displayCategory}
            </p>
          : null}
          <h3 className="text-pretty text-3xl font-semibold md:text-4xl">
            {title}
          </h3>
          {subtitle ?
            <p className="mt-1 text-pretty text-xl font-medium opacity-90">
              {subtitle}
            </p>
          : null}
          <p className="mt-4 text-pretty text-base/6 opacity-90">
            {description}
          </p>
          ...
          <div className="mt-8 space-y-5">
            {role ?
              <div>
                ...
                <p className="border-b border-foreground/20 pb-3 text-base/6">
                  {role}
                </p>
              </div>
            : null}
